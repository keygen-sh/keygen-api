...
ActiveRecord::Schema[8.1].define(version: 2026_04_01_150331) do
  ...
  # TABLE: event_logs
  # SQL: CREATE TABLE event_logs ( `id` UUID, `account_id` UUID, `environment_id` Nullable(UUID), `event_type_id` UUID, `request_log_id` Nullable(UUID), `created_at` DateTime64(3), `updated_at` DateTime64(3), `created_date` Date, `resource_type` LowCardinality(String), `resource_id` UUID, `whodunnit_type` LowCardinality(Nullable(String)), `whodunnit_id` Nullable(UUID), `idempotency_key` Nullable(String), `metadata` Nullable(JSON) TTL created_at + toIntervalDay(30), `is_deleted` UInt8 DEFAULT 0, `ver` DateTime64(3) DEFAULT now(), `ttl` UInt32 DEFAULT 2592000, INDEX idx_event_type event_type_id TYPE bloom_filter GRANULARITY 4, INDEX idx_resource (resource_type, resource_id) TYPE bloom_filter GRANULARITY 4, INDEX idx_whodunnit (whodunnit_type, whodunnit_id) TYPE bloom_filter GRANULARITY 4, INDEX idx_request_log request_log_id TYPE bloom_filter GRANULARITY 4, INDEX idx_idempotency idempotency_key TYPE bloom_filter GRANULARITY 4, INDEX idx_id id TYPE bloom_filter GRANULARITY 4 ) ENGINE = ReplacingMergeTree(ver, is_deleted) PARTITION BY toYYYYMM(created_date) ORDER BY (account_id, environment_id, created_date, UUIDToNum(id)) TTL created_at + toIntervalSecond(ttl) SETTINGS allow_nullable_key = 1, index_granularity = 8192
  create_table "event_logs", id: :uuid, options: "ReplacingMergeTree(ver, is_deleted) PARTITION BY toYYYYMM(created_date) ORDER BY (account_id, environment_id, created_date, UUIDToNum(id))", ttl: "created_at + toIntervalSecond(ttl)", settings: "allow_nullable_key = 1, index_granularity = 8192", force: :cascade do |t|
  ...
  create_table "event_logs_tmp", id: :uuid, options: "ReplacingMergeTree(ver, is_deleted) PARTITION BY toYYYYMM(created_date) ORDER BY (account_id, created_date, UUIDToNum(id))", ttl: "created_at + toIntervalSecond(ttl)", settings: "index_granularity = 8192", force: :cascade do |t|
    t.uuid "id", null: false
    t.uuid "account_id", null: false
    t.uuid "environment_id"
    t.uuid "event_type_id", null: false
