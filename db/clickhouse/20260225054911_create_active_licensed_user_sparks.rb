...
class CreateActiveLicensedUserSparks < ActiveRecord::Migration[8.1]
  def up
    create_table :active_licensed_user_sparks, id: false, options: 'MergeTree PARTITION BY toYYYYMM(created_date) ORDER BY (account_id, created_date)', force: :cascade do |t|
