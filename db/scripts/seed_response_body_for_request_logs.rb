...
Rails.logger.info "[scripts.seed_response_body_for_request_logs] Starting"
...
loop do
  ...
  count  = conn.update(<<~SQL.squish)
    ...
    FROM
      request_logs l
    LEFT OUTER JOIN
      request_log_blobs b ON b.blob_type      = 'response_body' AND
  ...
  Rails.logger.info "[scripts.seed_response_body_for_request_logs] Updated #{count} request log rows (batch ##{batch})"
