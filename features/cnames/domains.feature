...
Feature: Custom domains via CNAME
  ...
  Scenario: Product requests their profile using a custom domain (with CNAME)
    Given the current account is "example-com"
  ...
  Scenario: Product requests their profile using a custom domain (no CNAME)
