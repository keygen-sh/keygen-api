...
Before do
  algorithms = %w[ed25519 rsa-pss-sha256 rsa-sha256 ecdsa-secp256r1]
...
Then /^the response should contain a valid(?: "([^\"]+)")? signature header for "([^\"]+)"$/ do |expected_algorithm, account_id|
  ...
  begin
    ...
    if expected_algorithm.present?
      ...
      expect(algorithm).to satisfy { |v| %w[ed25519 rsa-pss-sha256 rsa-sha256 ecdsa-secp256r1].include?(v) }
    end
