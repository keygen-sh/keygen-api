...
Feature: Create account
  ...
  Scenario: Anonymous creates an account with autogenerated account information using a domain that is already used
    ...
    And the first error should have the following properties:
      """
      {
        ...
        "detail": "already exists for this domain (please use a different domain or use account recovery)",
        "source": {
  ...
  Scenario: Anonymous creates an account using a free email service
    When I send a POST request to "/accounts" with the following:
      """
      {
        "data": {
      ...
      """
    Then the response status should be "201"
    And the account "zeke" should have 1 "admin"
    And the account should not receive a "prompt-for-first-impression" email
    And sidekiq should have 0 "slack-invite" jobs

  Scenario: Anonymous creates an account using a disposable email service
    When I send a POST request to "/accounts" with the following:
      """
      {
        "data": {
              ...
              "data": [
                {
                  "type": "user",
                  "attributes": {
                    "email": "zeke@mailinator.com",
                    "password": "90853d98c25d"
                  }
                }
              ]
            }
  ...
  Scenario: Anonymous creates an account using a work email
    When I send a POST request to "/accounts" with the following:
      """
      {
        "data": {
    ...
    And the account "keygen-sh" should have 1 "admin"
    And the account should receive a "prompt-for-first-impression" email
    And sidekiq should have 1 "slack-invite" job
