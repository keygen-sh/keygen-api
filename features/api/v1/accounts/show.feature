...
Feature: Show account
  ...
  Scenario: Admin retrieves their account, accepting an ecdsa-secp256r1 signature algorithm
    Given I am an admin of account "test1"
    And I use an authentication token
    And I send the following headers:
      """
      { "Keygen-Accept-Signature": "algorithm=\"ecdsa-secp256r1\"" }
      """
    When I send a GET request to "/accounts/test1"
    And the response should contain a valid "ecdsa-secp256r1" signature header for "test1"
    Then the response status should be "200"
