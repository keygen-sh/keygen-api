...
Feature: List release artifacts
  ...
  Scenario: Admin retrieves an offset-paginated list of artifacts
    Given I am an admin of account "test1"
    And the current account is "test1"
    And the current account has 19 "artifacts"
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?page[number]=4&page[size]=5"
    Then the response status should be "200"
    And the response body should be an array with 4 "artifacts"
    And the response body should contain the following links:
      """
      {
        "self": "/v1/accounts/test1/artifacts?page[number]=4&page[size]=5",
        "prev": "/v1/accounts/test1/artifacts?page[number]=3&page[size]=5",
        "first": "/v1/accounts/test1/artifacts?page[number]=1&page[size]=5",
        "meta": {
          "pages": 4,
          "count": 19
        }
  ...
  Scenario: Admin retrieves an offset-paginated list of artifacts with a page size that is too high
    Given I am an admin of account "test1"
    And the current account is "test1"
    And the current account has 19 "artifacts"
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?page[number]=1&page[size]=250"
    Then the response status should be "400"

  Scenario: Admin retrieves an offset-paginated list of artifacts with a page size that is too low
    Given I am an admin of account "test1"
    And the current account is "test1"
    And the current account has 19 "artifacts"
    And I use an authentication token
  ...
  Scenario: Admin retrieves an offset-paginated list of artifacts with an invalid page number (negative)
    Given I am an admin of account "test1"
    And the current account is "test1"
    And the current account has 19 "artifacts"
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?page[number]=-1&page[size]=10"
    Then the response status should be "400"

  Scenario: Admin retrieves an offset-paginated list of artifacts with an invalid page number (type)
    Given I am an admin of account "test1"
    And the current account is "test1"
    And the current account has 19 "artifacts"
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?page%5Bnumber%5D%3D=0"
    Then the response status should be "400"

  Scenario: Admin retrieves an offset-paginated list of artifacts with a page number that is too high
    Given I am an admin of account "test1"
    And the current account is "test1"
    And the current account has 19 "artifacts"
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?page[number]=101&page[size]=100"
    Then the response status should be "400"

  Scenario: Admin retrieves an offset-paginated list of artifacts with a page number that is too low
    Given I am an admin of account "test1"
    And the current account is "test1"
    And the current account has 19 "artifacts"
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?page[number]=0&page[size]=100"
    Then the response status should be "400"

  Scenario: Admin retrieves a keyset-paginated list of artifacts (first page)
    Given I am an admin of account "test1"
    And the current account is "test1"
    And the current account has 19 "artifacts"
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?page[size]=5&page[cursor]="
    Then the response status should be "200"
    And the response body should be an array with 5 "artifacts"
    And the response body should contain the following links:
      """
      {
        "self": "/v1/accounts/test1/artifacts?page[cursor]=&page[size]=5",
        "next": "/v1/accounts/test1/artifacts?page[cursor]=$artifacts[-5]&page[size]=5"
      }
      """

  Scenario: Admin retrieves a keyset-paginated list of artifacts (next page)
    Given I am an admin of account "test1"
    And the current account is "test1"
    And the current account has 19 "artifacts"
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?page[size]=5&page[cursor]=$artifacts[-5]"
    Then the response status should be "200"
    And the response body should be an array with 5 "artifacts"
    And the response body should contain the following links:
      """
      {
        "self": "/v1/accounts/test1/artifacts?page[cursor]=$artifacts[-5]&page[size]=5",
        "next": "/v1/accounts/test1/artifacts?page[cursor]=$artifacts[-10]&page[size]=5"
      }
      """

  Scenario: Admin retrieves a keyset-paginated list of artifacts (last page)
    Given I am an admin of account "test1"
    And the current account is "test1"
    And the current account has 19 "artifacts"
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?page[size]=5&page[cursor]=$artifacts[-15]"
    Then the response status should be "200"
    And the response body should be an array with 4 "artifacts"
    And the response body should contain the following links:
      """
      {
        "self": "/v1/accounts/test1/artifacts?page[cursor]=$artifacts[-15]&page[size]=5",
        "next": null
      }
      """

  Scenario: Admin retrieves a keyset-paginated list of artifacts (no results)
    Given I am an admin of account "test1"
    And the current account is "test1"
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?page[size]=5&page[cursor]="
  ...
  Scenario: Admin retrieves a keyset-paginated list of artifacts (nonexistent cursor)
    Given I am an admin of account "test1"
    And the current account is "test1"
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?page[size]=5&page[cursor]=52a26820-0caa-4bac-9ff1-27bb6ec77ac4"
    Then the response status should be "200"
    And the response body should be an array with 0 "artifacts"
    And the response body should contain the following links:
      """
      {
        "self": "/v1/accounts/test1/artifacts?page[cursor]=52a26820-0caa-4bac-9ff1-27bb6ec77ac4&page[size]=5",
        "next": null
      }
      """

  Scenario: Admin retrieves a keyset-paginated list of artifacts (invalid cursor)
    Given I am an admin of account "test1"
    And the current account is "test1"
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?page[size]=5&page[cursor]=foo"
    Then the response status should be "400"
  ...
  Scenario: License retrieves a keyset-paginated list of artifacts (first page)
    Given the current account is "test1"
    And the current account has 1 "product"
    And the current account has 1 "policy" for the last "product"
    And the current account has 1 "release" for the last "product"
    And the current account has 19 "artifacts" for the last "release"
  ...
  Scenario: License retrieves a keyset-paginated list of artifacts (next page)
    Given the current account is "test1"
    And the current account has 1 "product"
    And the current account has 1 "policy" for the last "product"
    And the current account has 1 "release" for the last "product"
  ...
  Scenario: License retrieves a keyset-paginated list of constrained artifacts (first page)
    Given the current account is "test1"
    And the current account has 1 "product"
    And the current account has 1 "policy" for the last "product"
    And the current account has 1 constrained "release" for the last "product"
    And the current account has 19 "artifacts" for the last "release"
    And the current account has 1 entitled "license" for the last "policy"
    And I am a license of account "test1"
    And I authenticate with my key
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?page[size]=5&page[cursor]="
  ...
  Scenario: License retrieves a keyset-paginated list of constrained artifacts (next page)
    Given the current account is "test1"
    And the current account has 1 "product"
    And the current account has 1 "policy" for the last "product"
    And the current account has 1 constrained "release" for the last "product"
  ...
  Scenario: Admin retrieves all artifacts by release
    ...
    Then the response status should be "200"
    And the response body should be an array with 1 "artifact"
    And the first "artifact" should have the following relationships:
      """
      {
        "release": {
          "links": { "related": "/v1/accounts/$account/releases/0a027f00-0860-4fa7-bd37-5900c8866818" },
          "data": { "type": "releases", "id": "0a027f00-0860-4fa7-bd37-5900c8866818" }
        }
      }
      """

  Scenario: Admin retrieves all artifacts by release version
    Given the current account is "test1"
    And the current account has the following "product" rows:
      | id                                   | name   |
      | 6198261a-48b5-4445-a045-9fed4afc7735 | Test 1 |
    ...
    And I am an admin of account "test1"
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?release=1.0.0"
    Then the response status should be "200"
    ...
    And the first "artifact" should have the following relationships:
      ...
      """
    And the second "artifact" should have the following relationships:
      """
      {
        "release": {
          "links": { "related": "/v1/accounts/$account/releases/fffa0764-3a19-48ea-beb3-8950563c7357" },
          "data": { "type": "releases", "id": "fffa0764-3a19-48ea-beb3-8950563c7357" }
        }
      }
      """

  Scenario: Admin retrieves all artifacts by release tag
    Given the current account is "test1"
    And the current account has the following "product" rows:
      | id                                   | name   |
      | 6198261a-48b5-4445-a045-9fed4afc7735 | Test 1 |
      | 54a44eaf-6a83-4bb4-b3c1-17600dfdd77c | Test 2 |
    And the current account has the following "release" rows:
      | id                                   | product_id                           | version      | channel  | tag    |
      | fffa0764-3a19-48ea-beb3-8950563c7357 | 6198261a-48b5-4445-a045-9fed4afc7735 | 1.0.0        | stable   |        |
      | 165d5389-e535-4f36-9232-ed59c67375d1 | 6198261a-48b5-4445-a045-9fed4afc7735 | 1.0.1        | stable   |        |
      | e4fa628e-593d-48bc-8e3e-5e4dda1f2c3a | 6198261a-48b5-4445-a045-9fed4afc7735 | 1.1.0        | stable   | latest |
      | fd10ab0c-c52a-412f-b34f-180eebd7325d | 6198261a-48b5-4445-a045-9fed4afc7735 | 1.2.0-beta.1 | beta     |        |
      | f98d8c17-5fad-4361-ad89-43b0c6f6fa00 | 6198261a-48b5-4445-a045-9fed4afc7735 | 1.0.0-beta.1 | beta     |        |
      | 077ca1f2-6125-4a77-bdf0-3161a0fc278e | 6198261a-48b5-4445-a045-9fed4afc7735 | 1.0.0-beta.2 | beta     |        |
      | 0a027f00-0860-4fa7-bd37-5900c8866818 | 54a44eaf-6a83-4bb4-b3c1-17600dfdd77c | 1.0.0        | stable   |        |
    ...
    And I am an admin of account "test1"
    And I use an authentication token
    When I send a GET request to "/accounts/test1/artifacts?release=latest"
    Then the response status should be "200"
    And the response body should be an array with 1 "artifact"
    And the first "artifact" should have the following relationships:
      """
      {
        "release": {
          "links": { "related": "/v1/accounts/$account/releases/e4fa628e-593d-48bc-8e3e-5e4dda1f2c3a" },
          "data": { "type": "releases", "id": "e4fa628e-593d-48bc-8e3e-5e4dda1f2c3a" }
        }
      }
      """
