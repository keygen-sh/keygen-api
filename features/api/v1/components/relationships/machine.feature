...
Feature: Component machine relationship
  ...
  Scenario: User attempts to retrieve the machine for a component they own (license owner)
    Given the current account is "test1"
  ...
  Scenario: User attempts to retrieve the machine for a component they own (license user)
    Given the current account is "test1"
    And the current account has 1 "user"
