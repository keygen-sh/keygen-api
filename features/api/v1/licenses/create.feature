...
Feature: Create license
  ...
  Scenario: Admin creates a license using scheme ECDSA_SECP256R1_SIGN for a user of their account
    Given I am an admin of account "test1"
    And the current account is "test1"
    And the current account has 1 "webhook-endpoint"
    And the current account has 1 "product"
    ...
    And the first "policy" has the following attributes:
      """
      { "productId": "$products[0].id", "scheme": "ECDSA_SECP256R1_SIGN" }
      """
    And the current account has 1 "user"
    And I use an authentication token
    When I send a POST request to "/accounts/test1/licenses" with the following:
    ...
    Then the response status should be "201"
    And the current account should have 1 "license"
    And the response body should a "license" that contains a valid "ECDSA_SECP256R1_SIGN" key with the following dataset:
      """
      {
        "account": { "id": "$accounts[0].id" },
        "product": { "id": "$products[0].id" },
  ...
  Scenario: Admin creates a license using scheme ECDSA_SECP256R1_SIGN with a pre-determined key
    Given I am an admin of account "test1"
    And the current account is "test1"
    And the current account has 1 "webhook-endpoint"
    And the current account has 1 "policies"
    ...
    Then the response status should be "201"
    And the current account should have 1 "license"
    And the response body should be a "license" with the signed key of "ecdsa-signed-payload" using "ECDSA_SECP256R1_SIGN"
    And the response body should be a "license" with the scheme "ECDSA_SECP256R1_SIGN"
    And the response body should be a "license" that is not encrypted
    And sidekiq should have 1 "webhook" job
    And sidekiq should have 1 "metric" job
    And sidekiq should have 1 "request-log" job

  Scenario: Admin creates a license using scheme ECDSA_SECP256R1_SIGN with a short key
    Given I am an admin of account "test1"
    And the current account is "test1"
    And the current account has 1 "webhook-endpoint"
    And the current account has 1 "policies"
    ...
    Then the response status should be "201"
    And the current account should have 1 "license"
    And the response body should be a "license" with the signed key of "short" using "ECDSA_SECP256R1_SIGN"
    And the response body should be a "license" with the scheme "ECDSA_SECP256R1_SIGN"
    And the response body should be a "license" that is not encrypted
    And sidekiq should have 1 "webhook" job
    And sidekiq should have 1 "metric" job
  ...
  Scenario: Admin creates a license using scheme ECDSA_SECP256R1_SIGN using template variables
    Given I am an admin of account "test1"
    And the current account is "test1"
    And the current account has 1 "webhook-endpoint"
    And the current account has 1 "policies"
