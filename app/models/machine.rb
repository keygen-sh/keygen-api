...
class Machine < ApplicationRecord
  ...
  scope :alive, -> {
    ...
    idle_machines  = scp.where(last_heartbeat_at: nil, policies: { require_heartbeat: false })
  ...
  scope :dead, -> {
    ...
    expired_machines = scp.where(last_heartbeat_at: nil, policies: { require_heartbeat: true })
                          .where(<<~SQL.squish, Time.current, HEARTBEAT_TTL.to_i)
                            machines.created_at < ?::timestamp - INTERVAL '1 second' * COALESCE(policies.heartbeat_duration, ?)
                          SQL
