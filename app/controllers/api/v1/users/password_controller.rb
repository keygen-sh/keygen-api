module Api::V1::Users
  class PasswordController < Api::V1::BaseController
    ...
    def update_password
      ...
      if @user.try(:authenticate, password_params[:old_password])
    ...
    def reset_password
      ...
      if @user.compare_encrypted_token(:password_reset_token, password_params[:password_reset_token])
        ...
        if @user.update(password: password_params[:new_password], password_reset_token: nil, password_reset_sent_at: nil)
          render jsonapi: @user
        else
          render_unprocessable_resource @user
        end
